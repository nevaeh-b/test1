...
4. TRANSPORT_TICKET의 경우 기차표, KTX, SRT, 버스표, 항공권 등을 정상적인 교통 증빙으로 인정하라.
5. 기차표라고 판단할 수 있는 정보가 충분히 있다면 문서의 디자인이나 형식이 일반적인 기차표와 다르더라도 TRANSPORT_TICKET으로 인정하라.
6. 이미지가 약간 흐리거나 일부 글자가 잘리지 않았더라도 전체적인 증빙 종류를 판단할 수 있다면 인정할 수 있다.
...
10. 사용자가 선택한 documentType과 실제 이미지의 종류가 일치하면 valid는 true로 판단하라.
11. 사용자가 선택한 documentType과 실제 이미지의 종류가 다르면 valid는 false로 판단하라.
12. 반드시 JSON만 반환하라.

중요:

예를 들어 사용자가 TRANSPORT_TICKET을 선택했고 이미지가 KTX 승차권이라면:
{
  "valid": true,
  "documentType": "TRANSPORT_TICKET",
  "confidence": 0.95,
  "reason": "KTX 열차명과 승차권 정보가 확인되어 교통 이용 증빙으로 판단됩니다."
}
